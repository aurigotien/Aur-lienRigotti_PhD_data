...
do #{	
	...
	for k in `seq 1 13`; # create 13 velocity files 
        do #{
        	cd /data/failles/rigottia/Documents/stacks/$num_simu/run_$i/extr_data/velocity/
        	mkdir temp
		touch vel_$k.txt

		cd /data/failles/rigottia/Documents/stacks/$num_simu/run_$i/extr_data/velocity/temp/
		touch vel_$k.txt
	done #}
